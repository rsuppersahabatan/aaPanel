...
class SkillManager:
    ...
    def set_skill_enabled(self, name: str, enabled: bool) -> Dict:
        skill = self.get(name)
        if not skill:
            return {"status": False, "msg": lang("Skill does not exist: " + name)}
        disabled_names = self._disabled_name_set()
        if enabled:
        ...
        if not self._save_state():
            return {"status": False, "msg": lang("Failed to save skill state")}
        return {"status": True, "msg": lang("Settings saved successfully")}
